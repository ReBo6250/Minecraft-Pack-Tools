...
function openNewFile(oldFilePath, newFilePath) {
  // Close the current editor if it's associated with the modified file
  const activeEditor = vscode.window.activeTextEditor;
  if (activeEditor && activeEditor.document.uri.fsPath === oldFilePath) {
...
async function getFilesInFolder(parentFolderPath) {
  ...
  for (const file of files) {
    if (file.isDirectory()) {
      filePaths = [...filePaths, ...(await getFilesInFolder(`${parentFolderPath}/${file.name}`))];
    } else {
      filePaths.push(`${parentFolderPath}/${file.name}`);
    }
  }
  return filePaths;
}
